...
class FilteredDxfToPngConverter:
    ...
    def process_file(self, dxf_file, output_dir):
        ...
        # 根据输出目录名称确定SVG目录名称
        # 如果输出目录名称包含'png'，则替换为'svg'，否则添加'_svg'后缀
        if 'png' in output_dir_name:
            svg_dir_name = output_dir_name.replace('png', 'svg')
        else:
            svg_dir_name = f"{output_dir_name}_svg"
        
        # 创建SVG目录（与PNG目录同级）
        svg_dir = os.path.join(parent_dir, svg_dir_name)
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, required=True,
                        help='PNG输出目录，SVG将保存在同级的对应SVG目录中')
