...
def calculate_offset(ref_areas, target_areas):
    ...
    return final_lat_offset, final_lon_offset, offset_details
...
def calculate_alignment_metrics(ref_areas, target_areas, lat_offset, lon_offset):
    """
    计算对齐后的各种质量指标，包括 Residual Alignment Error

    参数：
        ref_areas: 参照图中的区域字典
        target_areas: 待校正图中的区域字典（应用偏移前的坐标）
        lat_offset: 已应用的纬度偏移量
        lon_offset: 已应用的经度偏移量

    返回：
        包含各种对齐质量指标的字典
    """
    residuals = []  # 存储每对匹配区域的残差
    residual_details = []  # 存储详细的残差信息
        ...
        if name in target_areas:
            ...
            for ref_area in ref_list:
                for target_area in target_list:
                    ...
                    if ref_area['level'] != target_area['level']:
                        # 计算应用偏移后的目标坐标
                        target_coords_aligned = [
                            (lat + lat_offset, lon + lon_offset)
                            for lat, lon in target_area['coordinates']
                        ]

                        # 计算质心
                        ref_centroid = calculate_centroid(ref_area['coordinates'])
                        target_centroid_aligned = calculate_centroid(target_coords_aligned)

                        if ref_centroid and target_centroid_aligned:
                            # 计算残差（质心距离）
                            lat_residual = ref_centroid[0] - target_centroid_aligned[0]
                            lon_residual = ref_centroid[1] - target_centroid_aligned[1]

                            # 转换为米
                            distance_m = latlon_to_meters(
                                ref_centroid[0], ref_centroid[1],
                                target_centroid_aligned[0], target_centroid_aligned[1]
                            )
                            ...
                            residual_details.append({
                                ...
                                'target_level': target_area['level'],
                                'ref_centroid': ref_centroid,
                                'target_centroid_aligned': target_centroid_aligned,
                                'lat_residual': lat_residual,
                                'lon_residual': lon_residual,
                                'distance_m': distance_m
                            })
                            ...
                            # 如果顶点数量相同，也计算顶点级别的残差
                            if len(ref_area['coordinates']) == len(target_coords_aligned):
                                for i in range(len(ref_area['coordinates'])):
                                    ref_lat, ref_lon = ref_area['coordinates'][i]
                                    target_lat, target_lon = target_coords_aligned[i]
                                    vertex_dist = latlon_to_meters(ref_lat, ref_lon, target_lat, target_lon)
                                    residual_details[-1].setdefault('vertex_residuals_m', []).append(vertex_dist)
    ...
    # 计算各种统计指标
    distances = [r['distance_m'] for r in residuals]
    lat_residuals = [r['lat_residual'] for r in residuals]
    lon_residuals = [r['lon_residual'] for r in residuals]
    ...
    rmse = math.sqrt(sum(d ** 2 for d in distances) / n)

    # 标准差
    std_distance = math.sqrt(sum((d - mean_distance) ** 2 for d in distances) / n) if n > 1 else 0
    ...
    # 90th percentile
    p90_index = int(n * 0.9)
    p90_distance = sorted_distances[min(p90_index, n - 1)]

    # 纬度和经度方向的误差统计
    lat_mean = sum(lat_residuals) / n
    lon_mean = sum(lon_residuals) / n
    lat_std = math.sqrt(sum((r - lat_mean) ** 2 for r in lat_residuals) / n) if n > 1 else 0
    lon_std = math.sqrt(sum((r - lon_mean) ** 2 for r in lon_residuals) / n) if n > 1 else 0

    metrics = {
        'num_matches': n,
        'residual_alignment_error': {
            'mean_m': mean_distance,
            'rmse_m': rmse,
            'max_m': max_distance,
            'min_m': min_distance,
            'std_m': std_distance,
            'median_m': median_distance,
            'p90_m': p90_distance,
        },
        'lat_residual': {
            'mean': lat_mean,
...
def print_alignment_metrics(metrics, target_file=None):
    """
    格式化输出对齐质量指标

    参数：
        metrics: calculate_alignment_metrics 返回的指标字典
        target_file: 目标文件名（可选）
    """
    print("\n" + "=" * 60)
    if target_file:
        print(f"对齐质量指标 - {os.path.basename(target_file)}")
    else:
        print("对齐质量指标 (Alignment Quality Metrics)")
    print("=" * 60)
    ...
    print(f"\n匹配区域数量: {metrics['num_matches']}")

    rae = metrics['residual_alignment_error']
    print(f"\n--- Residual Alignment Error (残差对齐误差) ---")
    print(f"  平均误差 (Mean):     {rae['mean_m']:.4f} 米")
    print(f"  均方根误差 (RMSE):   {rae['rmse_m']:.4f} 米")
    print(f"  最大误差 (Max):      {rae['max_m']:.4f} 米")
    print(f"  最小误差 (Min):      {rae['min_m']:.4f} 米")
    print(f"  标准差 (Std):        {rae['std_m']:.4f} 米")
    print(f"  中位数 (Median):     {rae['median_m']:.4f} 米")
    print(f"  90百分位 (P90):      {rae['p90_m']:.4f} 米")

    print(f"\n--- 坐标方向残差 ---")
    print(f"  纬度残差: 平均={metrics['lat_residual']['mean']:.12f}, 标准差={metrics['lat_residual']['std']:.12f}")
    print(f"  经度残差: 平均={metrics['lon_residual']['mean']:.12f}, 标准差={metrics['lon_residual']['std']:.12f}")

    # 打印每个匹配区域的详细残差
    print(f"\n--- 各匹配区域残差详情 ---")
    for detail in metrics['details']:
        print(f"  区域: {detail['name']}, 参照层: {detail['ref_level']}, 目标层: {detail['target_level']}")
        print(f"    残差距离: {detail['distance_m']:.4f} 米")
        if 'vertex_residuals_m' in detail:
            vertex_mean = sum(detail['vertex_residuals_m']) / len(detail['vertex_residuals_m'])
            vertex_max = max(detail['vertex_residuals_m'])
            print(f"    顶点残差: 平均={vertex_mean:.4f}米, 最大={vertex_max:.4f}米")

    print("=" * 60)


def save_alignment_metrics_to_json(all_metrics, output_path):
    """
    将所有对齐指标保存到JSON文件

    参数：
        all_metrics: 包含所有文件对齐指标的列表
        output_path: 输出JSON文件路径
    """
    # 将 details 中的坐标转换为可序列化格式
    serializable_metrics = []
    for entry in all_metrics:
        entry_copy = copy.deepcopy(entry)
        if 'metrics' in entry_copy and 'details' in entry_copy['metrics']:
            for detail in entry_copy['metrics']['details']:
                if 'ref_centroid' in detail and detail['ref_centroid']:
                    detail['ref_centroid'] = list(detail['ref_centroid'])
                if 'target_centroid_aligned' in detail and detail['target_centroid_aligned']:
                    detail['target_centroid_aligned'] = list(detail['target_centroid_aligned'])
        serializable_metrics.append(entry_copy)

    json_path = output_path.replace('.osm', '_alignment_metrics.json')
    if not json_path.endswith('.json'):
        json_path = output_path + '_alignment_metrics.json'
    ...
    for transport_type, name_groups in vertical_transports.items():
        for name, instances in name_groups.items():
            ...
            for i in range(len(sorted_instances) - 1):
                ...
                lower_node_element.set('version', '1')
                lower_node_element.set('action', 'modify')
                lower_node_element.set('visible', 'true')
                ...
                upper_node_element.set('version', '1')
                upper_node_element.set('action', 'modify')
                upper_node_element.set('visible', 'true')
                ...
                passage_element.set('version', '1')
                passage_element.set('action', 'modify')
                passage_element.set('visible', 'true')
...
def main():
    ...
    for i, target_file in enumerate(target_files):
        ...
        # 计算对齐质量指标 (应用偏移前计算，因为 target_areas 还保持原始坐标)
        alignment_metrics = calculate_alignment_metrics(ref_areas, target_areas, lat_offset, lon_offset)

        # 打印对齐质量指标
        print_alignment_metrics(alignment_metrics, target_file)

        # 存储指标
        all_alignment_metrics.append({
    ...
    current_max_ids = find_max_ids(merged_root)
    start_node_id_passages = current_max_ids.get('node', 0) + 1
    start_way_id_passages = current_max_ids.get('way', 0) + 1
    add_vertical_passages_to_root(merged_root, start_node_id_passages, start_way_id_passages)

    # 保存合并后的OSM文件
    if not save_osm_file(merged_tree, args.output):
    ...
    if all_alignment_metrics:
        save_alignment_metrics_to_json(all_alignment_metrics, args.output)

        # 打印汇总信息
        print("\n" + "=" * 60)
        print("对齐质量汇总 (Alignment Quality Summary)")
        print("=" * 60)
        total_matches = sum(m['metrics'].get('num_matches', 0) for m in all_alignment_metrics)
        all_rmse = [m['metrics']['residual_alignment_error']['rmse_m']
                    for m in all_alignment_metrics
                    if 'residual_alignment_error' in m['metrics']]
        if all_rmse:
            avg_rmse = sum(all_rmse) / len(all_rmse)
            max_rmse = max(all_rmse)
            print(f"  总匹配区域数: {total_matches}")
            print(f"  平均 RMSE: {avg_rmse:.4f} 米")
            print(f"  最大 RMSE: {max_rmse:.4f} 米")
        print("=" * 60)
