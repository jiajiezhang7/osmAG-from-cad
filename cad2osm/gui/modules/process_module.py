...
class ProcessWorker(QThread):
    ...
    def run_full_process(self):
        ...
        def monitor_step_progress(step_name, step_index):
            ...
            self.progress_updated.emit(current_step / total_steps, step_name)
        ...
        if os.path.isfile(self.input_path):
            ...
            if 1 not in skip_steps:
                monitor_step_progress("DWG转DXF", step_count / total_steps)
                if not preprocessor.process_dwg_to_dxf(self.input_path, dxf_file):
                    success = False
                else:
                    result_files.append(dxf_file)
                    step_count += 1
                    self.progress_updated.emit(step_count / total_steps, None)
            
            # 步骤2: DXF -> 过滤后的DXF
            if success and 2 not in skip_steps:
                monitor_step_progress("DXF过滤", step_count / total_steps)
            ...
            if success and 3 not in skip_steps:
                monitor_step_progress("DXF转SVG", step_count / total_steps)
            ...
            if success and 4 not in skip_steps:
                monitor_step_progress("SVG转PNG", step_count / total_steps)
            ...
            if success:
                ...
                self.step_progress_updated.emit(1.0, "")
                self.process_completed.emit(True, "处理完成", result_files)
            else:
            ...
            for i, dwg_file in enumerate(dwg_files, 1):
                ...
                self.progress_updated.emit(progress, f"进度: {progress:.1f}% ({i}/{total_files})")
    ...
    def run_semi_process(self):
        ...
        if os.path.isfile(self.input_path):
            ...
            # 创建必要的目录
            svg_dir = os.path.join(self.output_dir, "img/svg_semi")
            png_dir = os.path.join(self.output_dir, "img/png_semi")
            ...
            if success:
                result_files.append(svg_file)
                self.progress_updated.emit(0.5, None)
                self.step_progress_updated.emit(1.0, None)
            else:
                self.process_completed.emit(False, "DXF转SVG失败")
                return
            
            # 步骤2: SVG -> PNG
            self.progress_updated.emit(0.5, "SVG转PNG")
