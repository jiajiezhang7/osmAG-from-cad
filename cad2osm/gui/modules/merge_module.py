...
class MergeModule(QObject):
    ...
    def start_merge(self, ref_path, target_paths, output_path, params=None):
        """
        启动OSM合并流程
        
        参数:
            ref_path: 参照OSM文件路径
            target_paths: 目标OSM文件路径列表
            output_path: 输出OSM文件路径
            params: 处理参数字典
        """
        # 创建并启动工作线程
        self.worker = MergeWorker(ref_path, target_paths, output_path, params)
