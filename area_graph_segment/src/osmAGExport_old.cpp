    ...
    try {
        ...
        if (params.params["polygon_processing"]["small_room_merge"]) {
            ...
            if (mergeEnabled) {
                ...
                // 执行小房间合并
                mergeSmallAdjacentRooms(minRoomArea, maxMergeDistance);
            }
        }
...
void RMG::AreaGraph::mergeSmallAdjacentRooms(double minArea, double maxMergeDistance) {
    ...
    for (const auto& op : mergeOperations) {
        ...
        // 合并多边形
        targetRoom->polygon = mergePolygons(smallRoom->polygon, targetRoom->polygon);
        
        // 转移小房间的通道到目标房间
        transferPassages(smallRoom, targetRoom);
        
        // 从 originSet 中移除小房间
        originSet.erase(std::remove(originSet.begin(), originSet.end(), smallRoom), originSet.end());
    ...
    for (auto passage : passagesToRemove) {
        passageEList.erase(std::remove(passageEList.begin(), passageEList.end(), passage), passageEList.end());
        delete passage;
    }
